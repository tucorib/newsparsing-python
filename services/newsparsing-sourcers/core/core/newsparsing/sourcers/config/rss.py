...
def get_rss_source_url(rss_source):
    return get_configuration().get('sources.rss.%s.url' % rss_source,
                                   None)


def get_rss_source_database_name(rss_source):
    return get_configuration().get('sources.rss.%s.storage.name' % rss_source,
                                   None)


def get_rss_source_database_url(rss_source):
    return get_configuration().get('sources.rss.%s.storage.url' % rss_source,
                                   None)


def get_rss_source_sourcer(rss_source):
    return get_configuration().get('sources.rss.%s.sourcer' % rss_source,
                                   None)
